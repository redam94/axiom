...
def overview() -> Any:
    ...
    order = ("core data io infer identify estimands surface sim design calibrate meta "
             "diagnose build adapters viz report").split()
    blurb = {
        "core": "Specs, dimensions, intervals, expressions — the vocabulary everything else is written in.",
        ...
        "calibrate": "Fold a randomized result into an observational model, and log what that assumed.",
    ...
    layer = {
        "core": "foundation", "data": "foundation", "io": "foundation", "infer": "sampler seam",
        "identify": "domain", "estimands": "domain", "surface": "domain", "sim": "domain",
        "design": "pillar", "calibrate": "pillar", "meta": "pillar",
        "diagnose": "composition", "build": "composition", "adapters": "composition",
        "viz": "composition", "report": "composition",
    }
    ...
    for name in order:
        ...
        total += len(symbols)
        packages.append({
            "name": name, "n": len(symbols), "symbols": symbols,
            "blurb": blurb[name], "layer": layer[name],
        })
...
def identify() -> Any:
    ...
    from axiom.identify import (
        CausalGraph, identify, minimal_adjustment_sets, ols, two_stage_least_squares,
    ...
    def est(label: str, e: Any) -> dict[str, Any]:
        ci = e.ci(0.95)
        return {"label": label, "estimate": e.estimate, "se": e.se,
                "lower": ci.lower, "upper": ci.upper, "error": e.estimate - truth,
                "method": e.method, "covariates": list(e.covariates)}
    ...
    out["not_identified"] = {
        ...
        "truth": truth,
        "blocked": {"graph": blocked.to_text(), "status": vb.status,
                    "reason": vb.verdict.reason},
    }
    ...
    def est_iv(label: str, e: Any) -> dict[str, Any]:
        ci = e.ci(0.95)
        return {"label": label, "estimate": e.estimate, "se": e.se, "lower": ci.lower,
                "upper": ci.upper, "error": e.estimate - truth_iv, "method": e.method}

    out["instrument"] = {
        "graph": ivw.graph.to_text(),
        "status": vi.status, "route": vi.route, "instrument": vi.instrument,
        "truth": truth_iv,
    ...
    for ry in grid:
        row = []
        for rd in grid:
            b = bias_bounds(estimate=naive_h.estimate, se=naive_h.se, df=df,
                            r2_yz_dx=float(ry), r2_dz_x=float(rd), mass=0.95)
    ...
    bounds_rows = []
    for label, rd, ry in (("the confounder that is actually there", r2_dz_x, r2_yz_dx),
                          ("half as strong", r2_dz_x / 2, r2_yz_dx / 2),
                          ("a quarter as strong", r2_dz_x / 4, r2_yz_dx / 4)):
        b = bias_bounds(estimate=naive_h.estimate, se=naive_h.se, df=df,
                        r2_yz_dx=ry, r2_dz_x=rd, mass=0.95)
        bounds_rows.append({"label": label, "r2_dz_x": rd, "r2_yz_dx": ry, "bias": b.bias,
    ...
    out["sensitivity"] = {
        "estimate": naive_h.estimate, "se": naive_h.se, "truth": truth,
        "actual_bias": naive_h.estimate - truth,
        "rv": rv.rv, "rv_alpha": rv.rv_alpha, "r2_yd_x": rv.r2_yd_x,
        "q_true": q_true, "rv_q": rv_q.rv,
        "grid": grid.tolist(), "contour": contour,
        "bounds": bounds_rows,
        "tipping": {"decision_at_zero": tp.decision_at_zero,
                    "probability_at_zero": tp.probability_at_zero,
                    "bias": tp.bias, "certainty": 0.9,
                    "threshold": 1.0,
...
def design() -> Any:
    ...
    from axiom.design import (
        DecisionSpec, DesignCandidate, EconomicInputs, ValuePerOutcome, anchor_draws,
        difference_se, eig_gaussian, evaluate_candidate, evoi_gaussian, mde, pareto_front,
        perturb, power, power_curve, sample_size,
    )
    ...
    registry = standard_estimands(
        treatment=prior_world.spec.treatments[0], outcome=prior_world.spec.outcome,
    ...
    truth_contrast = float(prior_world.forward({"a": 60.0}).mean()
                           - prior_world.forward({"a": 0.0}).mean())
    ...
    out["prior"] = {
        "mean": prior_mean, "sd": prior_sd, "truth": truth_contrast,
    ...
    out["sizing"] = {
        "effect": effect, "sd": sd,
        "n": ss.n, "n_treated": ss.n_treated, "n_control": ss.n_control, "power": ss.power,
    ...
    decision = DecisionSpec(name="scale_up", threshold=6.0,
    ...
    out["value"] = {
        ...
        "evpi": ev.evpi, "evsi": ev.evsi, "preposterior_sd": ev.preposterior_sd,
        "numeraire": ev.numeraire,
        "threshold": decision.threshold,
        "value_per_outcome": VALUE_PER_OUTCOME, "program_units": PROGRAM_UNITS,
        # EVSI as the experiment gets more precise
        "se_grid": np.linspace(0.1, 2.5, 40).tolist(),
        "evsi_grid": [evoi_gaussian(decision, prior_mean, prior_sd, float(s)).evsi
                      for s in np.linspace(0.1, 2.5, 40)],
        "evpi_line": ev.evpi,
    }

    vpo = ValuePerOutcome(value=VALUE_PER_OUTCOME, outcome_unit="unit", numeraire="USD",
                          source="stated by the decision owner, 2026")
    ratio_draws = contrast_draws / 60.0
    economics = EconomicInputs(
        value_per_outcome=vpo, dose_per_period=60.0, discount_rate=0.01, dose_unit="USD",
        dose_cost_per_unit=1.0, marginal_value_ratio=float(ratio_draws.mean()),
    )
    candidates = [
        DesignCandidate(name="powered_holdout", method="difference_in_differences", n_units=ss.n,
                        n_periods=12, holdout_fraction=ss.n_control / ss.n,
                        experiment_se=se_experiment, cost=300.0, cooldown_periods=2),
        DesignCandidate(name="proposed_holdout", method="difference_in_differences",
                        n_units=proposed_n, n_periods=12, holdout_fraction=0.5,
                        experiment_se=difference_se(proposed_n, sd=sd), cost=550.0,
                        cooldown_periods=2),
        DesignCandidate(name="switchback", method="switchback", n_units=ss.n, n_periods=16,
                        holdout_fraction=0.5, experiment_se=0.8 * se_experiment, cost=420.0,
                        cooldown_periods=1),
    ]
    scores = [evaluate_candidate(c, decision, prior_mean=prior_mean, prior_sd=prior_sd,
                                 economics=economics) for c in candidates]
    front = {s.name for s in pareto_front(scores, objectives=("net_value", "-cost", "eig"))}
    out["leaderboard"] = {
        "rows": [{"name": s.name, "eig": s.eig, "evsi": s.evsi,
                  "opportunity_cost": s.opportunity_cost, "cost": s.cost,
                  "net_value": s.net_value, "power": s.power,
                  "on_front": s.name in front} for s in scores],
        "winner": max(scores, key=lambda s: s.net_value).name,
        "ledger_line": vpo.ledger_line().statement,
    }

    sens = []
    for parameter, grid in (("value_per_outcome", (0.25, 0.5, 1.0, 2.0, 4.0)),
                            ("experiment_se", (0.5, 0.75, 1.0, 1.5, 2.0))):
        table = perturb(candidates, decision, prior_mean, prior_sd, economics, parameter, grid=grid)
        sens.append({"parameter": parameter, "mode": table.mode, "grid": list(grid),
                     "winners": list(table.winners), "base_winner": table.base_winner,
...
def surface() -> Any:
    ...
    from axiom.surface import (
        Bounds, allocate, canonical_analysis, central_composite, frontier, response_band,
        fit, marginal_band, steepest_ascent,
    )
    ...
    truth = {"alpha": 2.0, "beta_a": 10.0, "k_a": 50.0, "s_a": 2.0,
             "beta_b": 6.0, "k_b": 12.0, "s_b": 1.5}
    from axiom.surface import HillKernel

    arms = arms_world(
        n_units=ccd.n, treatments=("a", "b"),
        kernels={"a": HillKernel(reference_dose=50.0, amplitude_scale=10.0),
    ...
    out["design"] = {
        "kind": ccd.kind, "n": ccd.n, "detail": jsonable(dict(ccd.detail)),
    ...
    for n in ("alpha", "beta_a", "k_a", "s_a", "beta_b", "k_b", "s_b", "sigma"):
        ...
        recovery.append({
            "name": n, "truth": t, "mean": s.mean, "sd": post.summary(n).sd,
            "lower": s.interval.lower, "upper": s.interval.upper,
    ...
    for name, hi in (("a", 100.0), ("b", 40.0)):
        ...
        entry = {
            "treatment": name, "doses": doses.tolist(),
            "mean": list(band.mean), "lower": list(band.lower), "upper": list(band.upper),
            "mass": 0.9,
            "truth": true_curve, "truth_base": base,
        }
        ...
        if hasattr(marg, "reason"):
            refusals.append({"call": f"marginal_band(result, {name!r}, doses=0..{hi:g})",
                             "type": type(marg).__name__, "reason": marg.reason})
        ...
        if not hasattr(marg, "reason"):
            entry.update({
                ...
                "marginal_mean": list(marg.mean), "marginal_lower": list(marg.lower),
    ...
    path = steepest_ascent(surf, theta_hat, {"a": 10.0, "b": 2.0}, step=5.0, n_steps=30,
                           bounds=bounds)
    out["ascent"] = {
        "stationary_kind": getattr(sp, "kind", None),
        "stationary_point": jsonable(getattr(sp, "point", None)),
        "n_steps": path.n, "stop": path.stop,
        "best": jsonable(path.best()),
        "points": [[float(p["a"]), float(p["b"])] for p in path.points]
        if isinstance(path.points[0], dict) else jsonable(path.points),
        "values": list(path.values),
    }

    BUDGET = 60.0
    alloc = allocate(surf, post, budget=BUDGET, bounds=bounds, objective="mean", seed=SEED)
    oracle = allocate(arms.surface, arms.theta, budget=BUDGET, bounds=bounds, objective="mean",
                      seed=SEED)
    ...
    out["allocation"] = {
        ...
        "fitted_outcome": alloc.expected_outcome, "status": alloc.status,
    ...
    z = [[float(arms.surface.forward({"a": float(a), "b": float(b)}, theta_hat).mean())
    ...
    out["checks"] = {
        "ppc": [{"name": s.name, "observed": s.observed, "lower": s.interval.lower,
                 "upper": s.interval.upper, "p": s.p_two_sided, "extreme": bool(s.extreme)}
        ...
        "unlearned": list(rep.unlearned), "saturated": list(rep.saturated),
...
def calibrate() -> Any:
    from axiom.calibrate import (
        aggregation_level, agreement, carryover_window_factor, derive_prior, fit_calibrated,
        resolve,
    )
    ...
    def contrast(name: str, window: TimeWindow, level: Level) -> Estimand:
        return Estimand(
            ...
            outcome=spec.outcome, population=Population(name="panel_units"), window=window,
            level=level, dimension=spec.outcome_dimension,
        )

    experiment_estimand = contrast("first_period_lift",
                                   TimeWindow(start=0, stop=1, basis="cumulative"),
                                   Level(unit="individual"))
    decision_estimand = contrast("steady_state_lift",
                                 TimeWindow(start=MAX_LAG, stop=world.n_periods,
                                            basis="per_period"),
                                 Level(unit="cluster"))
    ...
    out["setup"] = {
        ...
        "measurement": {"estimate": measurement.estimate, "se": measurement.se,
                        "lower": measurement.interval.lower,
                        "upper": measurement.interval.upper,
                        "method": measurement.method, "source": measurement.source,
                        "target_hash": measurement.target[:12]},
        "n_units": world.n_units, "n_periods": world.n_periods, "max_lag": MAX_LAG,
    }

    def moments(result: Any) -> dict[str, list[float]]:
        return {n: [float(result.posterior.flat(n).mean()),
                    float(result.posterior.flat(n).std(ddof=1))]
    ...
    calibrated = derive_prior([measurement], spec, "a", beta_draws=beta_draws,
                              contribution_draws=contribution_draws)
    prior_route = fit(calibrated.spec, observed, backend="laplace", draws=1000, seed=1)
    likelihood_route = fit_calibrated(spec, observed, [measurement], backend="laplace",
                                      draws=1000, seed=1)

    routes = []
    for label, result in (("uncalibrated", biased), ("prior route", prior_route),
        ...
        routes.append({
            "label": label,
            "moments": m,
            "beta_a": m["beta_a"][0], "beta_a_sd": m["beta_a"][1],
            "sd_above_truth": (m["beta_a"][0] - TRUTH["beta_a"]) / m["beta_a"][1],
            "realized": ag.posterior_mean, "posterior_sd": ag.posterior_sd,
            "z": ag.z, "verdict": ag.verdict,
        })
    out["routes"] = {
        "rows": routes,
        "truth": TRUTH,
        ...
        "prior_ledger": [{"kind": ln.kind, "assumption": ln.assumption.name,
                          "statement": ln.statement} for ln in calibrated.ledger_lines],
    ...
    window = carryover_window_factor(GeometricCarryover(max_lag=MAX_LAG), {"lam_a": lam_hat}, 1,
                                     treatment="a")
    level = aggregation_level(experiment_estimand.level, decision_estimand.level,
                              cluster_size=world.n_units, icc=0.05)
    ...
    out["transfer"] = {
        ...
        "source": experiment_estimand.name, "target": decision_estimand.name,
        "window_factor": window.value, "window_counterfactual": window.counterfactual,
        "lam_hat": lam_hat,
        "level_se_factor": level.value,
        "level_point_factor": jsonable(level.detail["point_factor"]),
        "status": resolved.status, "licensed": bool(resolved.licensed),
        "factor": resolved.factor, "se_scale": resolved.se_scale,
        "per_member": per_member, "per_member_se": per_member_se,
        ...
        "ledger": jsonable(lframe[["facet", "assumption", "state", "counterfactual", "value",
                                   "correction"]].to_dict("records")),
...
def meta() -> Any:
    ...
    from axiom.meta import (
        Corpus, EpsilonLedger, PoolPriors, PoolSpec, PrivacyPolicy, baujat, cell_from_records,
        check_cell, delta_identification, egger, forest_data, funnel_data, heterogeneity,
        leave_one_out, normalize, pool, prediction_interval, prior_from_pool, random_effects,
        record_from_summary, release,
    )
    ...
    for study, lab, read, est, se_, n in rows:
        mb = mb.study(study, estimate=est, se=se_, read=read, contributor=lab,
                      quantity="standardized_contrast", n=n,
                      moderators={"follow_up": float(rng.integers(4, 13))}, source=f"report-{study}")
    eleven = mb.build()

    per_acre = record_from_summary(
        study="s12", contributor="lab_j", quantity="standardized_contrast", estimate=1.32,
        se=0.20, read="experiment", family="fertilizer", n=80, unit_scale="per_acre",
        moderators={"follow_up": 6.0}, source="report-s12",
    )
    ...
    converted = s12.model_copy(update={
        "estimate": s12.estimate / REFERENCE_AREA, "se": s12.se / REFERENCE_AREA,
        "unit_scale": "", "detail": {**s12.detail, "converted_by": f"/ {REFERENCE_AREA} acre"},
    })
    ...
    for method in ("dl", "pm", "reml"):
        ...
        methods.append({"method": method, "estimate": re.estimate, "se": re.se, "tau2": re.tau2,
                        "lower": re.interval.lower, "upper": re.interval.upper})
    ...
    out["classical"] = {
        ...
        "studies": [{"study": r.study, "contributor": r.contributor, "read": r.read,
                     "estimate": r.estimate, "se": r.se, "n": r.n} for r in corpus.records],
        "methods": methods,
        "heterogeneity": {"q": het.q, "df": het.df, "p": het.p_value, "i2": het.i2,
                          "tau2": getattr(het, "tau2", None)},
        "knapp_hartung": {"estimate": kh.estimate, "se": kh.se,
                          "lower": kh.interval.lower, "upper": kh.interval.upper},
        "prediction_interval": [pi.lower, pi.upper],
        "egger": {"intercept": eg.intercept, "se": eg.se, "t": eg.t, "df": eg.df, "p": eg.p},
        "leave_one_out": [{"study": r.study, "influence": float(v)}
                          for r, v in zip(corpus.records, loo.influence)],
        "baujat": {"q_contribution": jsonable(bj.q_contribution),
                   "influence": jsonable(bj.influence),
                   "labels": [r.study for r in corpus.records]},
        "forest": {
            "rows": [{"label": row.label, "estimate": row.estimate, "se": row.se,
                      "lower": row.interval.lower, "upper": row.interval.upper,
            ...
            "pooled": {"estimate": fd.pooled_estimate, "lower": fd.pooled_interval.lower,
                       "upper": fd.pooled_interval.upper},
            "pooled_estimate": fd.pooled_estimate, "pooled_se": fd.pooled_se,
        ...
        "funnel": {
            "y": jsonable(y), "se": jsonable(se),
            "labels": [r.study for r in corpus.records],
            "read": [r.read for r in corpus.records],
            "pooled": fu.pooled,
            "contours": [{"mass": c.mass, "se": jsonable(c.se),
                          "lower": jsonable(c.lower), "upper": jsonable(c.upper)}
    ...
    out["pooled"] = {
        ...
        "mu": {"mean": res.mu.mean, "sd": res.mu.sd,
               "lower": res.mu.interval.lower, "upper": res.mu.interval.upper},
        "tau": {"mean": res.tau.mean, "sd": res.tau.sd},
        "delta": {"mean": res.delta.mean, "sd": res.delta.sd,
    ...
    out["handoff"] = {
        ...
        "kind": handoff.kind, "statement": handoff.statement,
        "prior_mean": float(np.exp(amplitude_prior.hyper["mu"]
                                   + amplitude_prior.hyper["sigma"] ** 2 / 2)),
    }

    cell = cell_from_records(corpus.records, name="fertilizer/standardized_contrast")
    policy = PrivacyPolicy(k=3, dominance_p=0.5, dominance_top_n=2, dominance_top_p=0.8,
                           epsilon_total=1.0)
    gate = check_cell(cell, policy)
    published, budget = release(cell, policy, EpsilonLedger(budget=policy.epsilon_total),
                                release_id="fertilizer-2026Q3", epsilon=0.5, clip=(0.0, 1.5),
                                seed=0, mechanism="laplace")
    out["privacy"] = {
        "gate": {"status": gate.status, "route": gate.route,
                 "contributors": len(cell.contributors)},
        "policy": {"k": policy.k, "epsilon_total": policy.epsilon_total},
        "released": published.value,
        "lower": published.interval.lower, "upper": published.interval.upper,
        "clipped_mean": float(np.mean(np.clip(cell.values, 0, 1.5))),
        "mechanism": published.mechanism, "noise_scale": published.noise_scale,
        "epsilon": published.epsilon, "used": jsonable(budget.used),
...
def casestudy() -> Any:
    ...
    from axiom.design import (
        LookSchedule, StoppingRule, crossing_probabilities, difference_se, harm_boundary,
        information_fractions, monitor, operating_characteristics,
    )
    ...
    for stratum, arm in CONTRASTS:
        ...
        se = difference_se(n_dose + n_control, sd=SD_WINDOW,
    ...
    for (stratum, arm), path in paths.items():
        ...
        monitoring.append({
            ...
            "arm": arm, "arm_label": h.ARM_LABEL[arm],
            "looks_taken": len(path.looks), "decision": path.decision,
    ...
    for s in h.STRATA:
        curves.append({
            "stratum": s, "label": h.STRATUM_LABEL[s],
    ...
    return {
        "protocol": {
            "n_units": h.N_UNITS, "arms": list(h.ARMS),
            "arm_labels": {a: h.ARM_LABEL[a] for a in h.ARMS},
            "dose": {a: h.DOSE[a] for a in h.ARMS},
            "dose_unit": h.DOSE_UNIT, "outcome_unit": h.OUTCOME_UNIT,
            ...
            "enrollment_weeks": h.ENROLLMENT_WEEKS, "follow_up_weeks": h.FOLLOW_UP_WEEKS,
            "primary_week": h.PRIMARY_WEEK,
            "n_arm": N_ARM,
        },
        "rule": {
            "look_weeks": LOOK_WEEKS, "information": list(INFORMATION),
            "n_looks": schedule.n_looks, "n_contrasts": len(rules),
            "margin": HARM_MARGIN, "probability": HARM_PROBABILITY, "sd_window": SD_WINDOW,
        },
        "monitoring": monitoring,
        "panels": panels,
        "stop": {
            "decision": stopped.decision,
            "look": stopped.stopped_at + 1, "n_looks": schedule.n_looks,
            "week": LOOK_WEEKS[stopped.stop.look],
            "information": stopped.information_used,
            "effect": stopped.stop.effect, "se": stopped.stop.se,
            "n": series[LOOK_WEEKS[stopped.stop.look]][("age_51_plus", "dose_40")][2],
            "z": stopped.stop.z, "threshold": stopped.threshold(),
        ...
        "truth": {
            "doses": doses.tolist(), "curves": curves,
            "pooled_itt": pooled_itt, "pooled_pp": pooled_pp, "week": week,
